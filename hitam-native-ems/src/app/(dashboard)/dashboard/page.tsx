...
export default async function DashboardPage({
  ...
  const now = new Date();
  let metric1Val: string | number = 0;
  let metric2Val: string | number = "-";
  let metric3Val: string | number = 0;
  
  let recentEvents: any[] = [];
  let pendingApprovals: any[] = [];

  if (role === 'STUDENT') {
    // Metric 1: Active Campus Events (Events with at least one session happening now)
    const allPublished = await db.query.events.findMany({
      where: eq(schema.events.status, "PUBLISHED"),
      with: { sessions: true }
    });
    
    const activeEvents = allPublished.filter(event => 
      event.sessions.some(s => new Date(s.startTime) <= now && new Date(s.endTime) >= now)
    );
    
    // Metric 3: Upcoming Sessions (Future sessions for my registrations)
    const myRegistrations = await db.query.registrations.findMany({
      where: eq(schema.registrations.studentId, freshUser.id),
      with: { event: { with: { sessions: true } } }
    });
    
    const upcomingSessions = myRegistrations.flatMap(r => r.event.sessions).filter(s => new Date(s.startTime) > now);

    metric1Val = activeEvents.length;
    metric3Val = upcomingSessions.length;
    metric2Val = "High"; // Affinity Placeholder

    recentEvents = allPublished.slice(0, 3);
  } 
    ...
    // Metric 1: My Active Sessions
    const activeSessions = myEvents.flatMap(e => e.sessions).filter(s => 
      new Date(s.startTime) <= now && new Date(s.endTime) >= now
    );
    
    // Metric 3: My Upcoming Events (Events with future sessions)
    const upcomingEvents = myEvents.filter(e => 
      e.sessions.some(s => new Date(s.startTime) > now)
    );

    metric1Val = activeSessions.length;
    metric3Val = upcomingEvents.length;
    recentEvents = myEvents.slice(0, 3);
    ...
    // Supervisory: Dept/Global metrics
    const allEvents = await db.query.events.findMany({
      with: { sessions: true }
    });

    const activeSessions = allEvents.flatMap(e => e.sessions).filter(s => 
      new Date(s.startTime) <= now && new Date(s.endTime) >= now
    );
    ...
    // LEAD_SE Specific: Categorize Metrics
    if (role === 'LEAD_SE') {
      const allPendingForLead = await db.query.eventApprovals.findMany({
        where: and(eq(schema.eventApprovals.approverRole, "LEAD_SE"), eq(schema.eventApprovals.status, "PENDING_APPROVAL")),
        with: { event: true }
      });
      
      metric1Val = allPendingForLead.filter(p => p.event?.category === 'CLUB').length;
      metric2Val = allPendingForLead.filter(p => p.event?.category === 'INSTITUTE').length;
      metric3Val = activeSessions.length; // Affinity Placeholder
    } else {
      metric1Val = activeSessions.length;
      metric2Val = pending.length;
      metric3Val = upcomingEvents.length;
    ...
    // PH Specific: Add Metric for pending ERP Syncs
    if (role === 'PROGRAM_HEAD') {
      try {
        const pendingSyncs = await db.query.attendanceSubmissions.findMany({
          where: eq(schema.attendanceSubmissions.isApproved, false),
          limit: 1
        });
        metric3Val = pendingSyncs.length > 0 ? "REVIEW PENDING" : "ALL SYNCED";
      } catch (e) {
        console.error("Dashboard Sync Check Failed (Schema update likely pending):", e);
        metric3Val = "INITIALIZING...";
      }
    }
  }
  else if (role === 'ADMIN') {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {/* Action Queue Section (Supervisory Only) */}
          {['PROGRAM_HEAD', 'HOD', 'LEAD_SE', 'AO'].includes(role) && (
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Action Queue</h3>
                <Link href="/approvals" className={cn("font-black text-[10px] uppercase tracking-widest hover:underline", config.theme.text)}>View All</Link>
              </div>
              
              <div className="space-y-4">
                {pendingApprovals.length > 0 ? (
                  pendingApprovals.map((approval) => (
                    <div key={approval.id} className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 p-5 rounded-[2rem] flex items-center gap-5 group hover:border-slate-300 transition-all shadow-sm">
                      ...
                      <div className="flex-grow min-w-0">
                        ...
                        <p className="text-xs font-semibold text-slate-400 truncate flex items-center gap-1">
                          Proposed by <span className="text-blue-500 font-black">{approval.event?.creator?.name || 'Unknown'}</span>
                        </p>
                      </div>
                      <Link href={`/approvals`} className={cn("p-3 rounded-xl transition-colors shrink-0", config.theme.accent, "text-white shadow-lg")}>
                        <span className="material-symbols-outlined">chevron_right</span>
                      </Link>
                    </div>
                  ))
                ) : (
                  <div className="p-10 text-center bg-white dark:bg-white/5 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-white/10">
                    <span className="material-symbols-outlined text-4xl text-slate-300 mb-2">check_circle</span>
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Your queue is clear</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Recent Events Section (Always show for all roles as a discovery feed) */}
          <div className="space-y-6">
            <div className="flex items-center justify-between px-2">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">
                {['STUDENT_COORDINATOR', 'FACULTY'].includes(role) ? "My Recent Work" : "Published Events"}
              </h3>
              <Link href="/events" className={cn("font-black text-[10px] uppercase tracking-widest hover:underline", config.theme.text)}>Explore All</Link>
            </div>
            
            <div className="space-y-4">
              {recentEvents.length > 0 ? (
                recentEvents.map((event) => (
                  <div key={event.id} className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 p-5 rounded-[2rem] flex items-center gap-5 group hover:border-slate-300 transition-all shadow-sm">
                    ...
                    <Link href={`/events/${event.id}`} className={cn("p-3 rounded-xl transition-colors shrink-0", config.theme.accent, "text-white shadow-lg")}>
                      <span className="material-symbols-outlined">chevron_right</span>
                    </Link>
                  </div>
                ))
                ...
                <div className="p-10 text-center bg-white dark:bg-white/5 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-white/10">
                  ...
                  <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">No events found</p>
                </div>
              )}
            </div>
          </div>
